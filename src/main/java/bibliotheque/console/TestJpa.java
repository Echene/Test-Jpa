...
public class TestJpa {
	...
	public static void main(String[] args) {
		...
		TypedQuery<Emprunt> queryEmprunts = em.createQuery("SELECT e FROM Emprunt e INNER JOIN FETCH e.livres", Emprunt.class);
		
		List<Emprunt> emprunts = queryEmprunts.getResultList();
		
		for(Emprunt e: emprunts) {
			
			LOGGER.info("Emprunt : " + e.getId() + "  |  Date début : " + e.getDateDebut() + "  |  Date fin : " + e.getDateFin()+ "\n");
